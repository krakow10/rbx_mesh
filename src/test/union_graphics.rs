use crate::union_graphics::{Error, MeshData, CSGMDL};

fn get_version(union_graphics: &MeshData) -> &str {
...
fn read_union_graphics(data: &[u8]) -> Result<MeshData, Error> {
	let mut cursor = std::io::Cursor::new(data);
	let union_graphics = crate::read_union_graphics_versioned(&mut cursor)?;
	assert_eq!(cursor.position(), data.len() as u64);
	Ok(union_graphics)
}
fn dbg_union_graphics(union_graphics: MeshData, expected_version: &str) {
	...
	match union_graphics {
		...
		MeshData::CSGMDL(CSGMDL::V4(csgmdl4)) => {
			println!("==V4");
		...
		MeshData::CSGMDL(CSGMDL::V5(csgmdl5)) => {
			println!("===V5===");
			println!("{:?}", csgmdl5);
